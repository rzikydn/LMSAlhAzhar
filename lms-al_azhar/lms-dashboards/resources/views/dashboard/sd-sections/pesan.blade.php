<div class="sd-content sd-content-pesan">
  <div class="content-header"><h1>Pesan</h1><div class="header-right"><label @click="showTulisPesan = !showTulisPesan" class="header-btn primary" style="cursor:pointer"><i class="fas fa-envelope"></i> Pesan Baru</label><div class="avatar teal">{{ strtoupper(substr($siswa->nama, 0, 1)) }}</div></div></div>
  ...
  <div class="card"><div class="card-header"><h3><i class="fas fa-inbox" style="color:var(--blue)"></i> Kotak Masuk</h3></div>
    ...
    <tbody>
      @forelse($pesan as $m)
      <tr>
        <td><strong>{{ $m->pengirim->name }}</strong> <span class="badge light blue">Guru</span></td>
        <td>{{ $m->isi }}</td>
